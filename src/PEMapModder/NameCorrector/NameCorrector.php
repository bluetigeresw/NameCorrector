...
namespace PEMapModder\NameCorrector;
...
class NameCorrector extends PluginBase implements Listener{
	...
	public function onEnable(){
        ...
        $apiVersionNumber = substr($apiVersion, 0, 5);
        /** @var string $apiVersionNumber */

        $this->getLogger()->notice("API Version: " . $apiVersion . " ");
        /*$this->getLogger()->notice("Checking API version for legacy API requirements (Pre-API 2.0.0)");*/
        /*$this->getLogger()->notice("[API Version Number]: " . $apiVersionNumber . " ");*/


        /*if($apiVersionNumber === "3.0.0") {
            $this->getLogger()->notice("API Version does not require legacy support.");
        } else {
            $this->getLogger()->notice("API Version[" . $apiVersionNumber ."] requires Legacy API support. Enabling...");
            $this->getLogger()->notice("Legacy Mode Enabled.");
            $legacyInfo =  new pocketmine\network\protocol\Info;
            $legacyMode = true;
        }*/
...
//			var_dump($pk->username);
	...
	public function correctName($name) : string{
